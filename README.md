...
Pushes to `main` publish the site. One-off setup in the repository:

1. **Settings → Pages → Build and deployment → Source: GitHub Actions.**
2. Put the resulting URL in `base-url` in `book/book.toml`, so canonical
   links, Open Graph tags and `sitemap.xml` are absolute. (Pages passes the
   URL to the build too, so this is belt and braces.)
