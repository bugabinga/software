...
(function () {
  ...
  const headings = Array.from(document.querySelectorAll(".chapter h2[id], .chapter h3[id]"));
  if (headings.length && "IntersectionObserver" in window) {
    const sectionLinks = new Map();
    document.querySelectorAll('.toc-sections a[href^="#"], .page-toc a[href^="#"]').forEach(
      function (link) {
    ...
    const observer = new IntersectionObserver(
      function (entries) {
        ...
        if (id === active) return;
        if (active) {
          (sectionLinks.get(active) || []).forEach((l) => l.classList.remove("active"));
        }
        active = id;
  ...
  function search(query, corpus) {
    ...
    for (const entry of corpus) {
      ...
      for (const term of terms) {
        ...
        const section = entry.sections.find((s) => s.title.toLowerCase().includes(term));
        if (section) {
          score += 6;
