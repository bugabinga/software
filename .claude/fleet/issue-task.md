...
Read the full issue as well — the form has fields the text above does not carry,
and the thread may have more:
...
If the request is unclear, or achieving it would take you outside your brief, do
not improvise a nearby task. Comment on the issue saying what you would need,
and stop. A request that comes back as a question costs less than one that comes
back as the wrong work.

Put `Closes #{{issue}}` in your commit message so the issue closes when this
merges. `mise run check` before pushing. Push to the branch named above and
stop.
