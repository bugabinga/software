...
`mise run check-code` type-checks every file in this directory, and CI fails if
a listing stops compiling.
...
dedented, with the marker lines removed. Markers go inside comments, so the file
still compiles:
...
To add a language, add a branch to `tools/check_code.sh`. Rust files are checked
with `rustc --emit=metadata`, which needs no Cargo project.
